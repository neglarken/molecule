...
.. seealso::
    ...
    identifier: str = ''
    "identifier() -> str\n\nThe unique identifier string of the data object. This identifier serves as a lookup key in object dictionaries,\nsuch as the :py:attr:`DataCollection.tables` collection, or as a target name in various places where a data object needs to be referenced by name,\nsuch as the `TimeAveragingModifier.operate_on` field.\n\nThe identifier string must not contain any slashes ('/') or colons (':').\n\nData objects generated by modifiers in a pipeline typically have an automatically assigned identifier, as documented in the description of the respective modifier.\nWhen writing your own modifier function, it is your responsibility to give new data objects created by your modifier function a meaningful identifier.\nThis allows subsequent modifiers in the pipeline to refer to these data objects."
    vis: Optional[ovito.vis.DataVis] = None
    "vis() -> Optional[ovito.vis.DataVis]\n\nThe :py:class:`DataVis` element currently associated with this data object, which is responsible for\nvisually rendering the stored data. If set to ``None``, the data object remains non-visual and doesn't appear in\nrendered images or the viewports. Furthermore, note that the same :py:class:`DataVis` element may be assigned to\nmultiple data objects in order to synchronize their visual appearance."

    def make_mutable(self, subobj: DataObjectType) -> DataObjectType:
        """This helper method requests a deep copy of *subobj*, which must be a child :py:class:`DataObject` of this parent :py:class:`DataObject`.
A copy will only be made in case the sub-object is currently referenced by at least one more parent object.
If, however, the sub-object is exclusively owned by this :py:class:`DataObject`, no copy is made and the original sub-object is returned as is.
The returned object is safe to modify without unexpected side effects, because any shared ownership is converted to an exclusive ownership by the method.

See :ref:`underscore_notation` for a discussion of object ownership and typical use cases for this method.
...
.. seealso:: :ref:`manual:scene_objects.particle_types`"""
...
.. seealso:: :py:meth:`count_manifolds`"""
        ...
...
.. seealso:: :py:attr:`next_manifold_edge`"""
        ...
