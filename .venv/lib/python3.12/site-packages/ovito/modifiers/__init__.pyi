...
from typing import Tuple, Optional, Any, Union, MutableSet, Sequence, Callable, Generator
import ovito.vis
...
In many cases you can replace the use of a :py:class:`ComputePropertyModifier` in a pipeline with an equivalent Python modifier function
  ...
  which can be accomplished more easily in the Python language and NumPy array computations.
  ...
  def compute_particle_colors(frame, data):
      cell_size_x = data.cell[0,0]
      rgb = np.empty((data.particles.count, 3))
      rgb[:, 0] = data.particles['Position'][:, 0] / cell_size_x
      rgb[:, 1] = 0.0
      rgb[:, 2] = 0.5
...
```python
  ...
  modifier.vis.shading = LinesVis.Shading.Flat
